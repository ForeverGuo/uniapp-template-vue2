  ...
  <div class="custom" :style="{ background: back_color }">
    ...
    >
      <div class="fs15 tc h_top" :style="{ background: back_color }">
        <div v-if="is_show" class="left_icon" :class="{ left_icon2: show_back || show_home }">
          <span v-if="back" class="back" @click.stop="handleBack"></span>
          <span v-if="back && home" class="line">|</span>
          <span v-if="home" class="home" @click.stop="handleHome"></span>
        </div>
...
export default {
  ...
  mounted() {
    ...
    uni.getSystemInfo({
      success: (res) => {
        ...
        const { top, height } = menuButton // top 可用于区分机型
        console.log('top:' + top)
        const { model } = res
        ...
        let barPaddingHeight,
          navHeight = height * 2.5,
          botMenuHeight
  ...
  methods: {
    ...
    hairScreen(res) {
      const { model, system, platform } = res
